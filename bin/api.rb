...
def create_bills_by_issue(issue)
...
def create_congress_members
  ...
  senator_array.each do |senator|
    CongressMember.create(name: "#{senator['first_name']} #{senator['last_name']}", party: senator["party"], state: senator["state"], title: senator["short_title"])
  end
end